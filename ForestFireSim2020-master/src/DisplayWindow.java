...
public class DisplayWindow {
    ...
    public void drawGrid(Tree[][] grid) {
        ...
        for (int i = 0; i < grid.length; i++) {
            for (int j = 0; j < grid[0].length; j++) {
                ...
                Tree thing = grid[i][j];
                if(thing.type != 1 && thing.type != 0){
                    displayColor = getColor(thing.type);
                }
                else if(thing.type == 1){
                    int burntime = thing.burnTime;

                    int rO = 0;
                    int gO = 170;
                    int bO = 0;

                    int r = 255 - (int)( Math.random() * 50 );
                    int g = (int)( gO - (gO * ( (double)burntime / 30) ) );
                    int b = (int)( bO - (bO * ( (double)burntime / 30) ) );
                    int rgb = new Color(r, g, b).getRGB();

                    displayColor = rgb;
                }else{
                    int greenery = thing.greenery;

                    int rgb = new Color(0, 255-greenery*20, 0).getRGB();
